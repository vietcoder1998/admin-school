    ...
    <div className="candidate-info">
      <div className="candidate-info-header ">
        ...
        <img
          ...
          src={
            !onErrCover && data && data.coverUrl ? data.coverUrl : backGround
          }
          alt={"base"}
        ...
        <div className="block-image">
          <div className="content-avatar">
            <Avatar
              // @ts-ignore
              src={
                !onErrLogo && data && data.avatarUrl ? data.avatarUrl : avatar
              }
              style={{
                height: "8vw",
                width: "8vw",
          ...
          <div className={"name-candidate"}>
            ...
            <Icon
              ...
              theme="filled"
              style={{
                color: data && data.profileVerified ? "greenyellow" : "red",
              }}
            />
          </div>
        </div>
            ...
            <Row>
              <Col sm={24} md={24} lg={24} xl={24} xxl={24}>
                <Card title="Trình độ ngoại ngữ" bordered={false}>
                  ...
                  data.languageSkills.length > 0 ? (
                    data.languageSkills.map((item?: any, i?: number) => (
                      <div key={i}>
                        <div>
                          <IptLetter value="Ngôn ngữ: " />
                          {item.language.name}
                        </div>
                        <div>
                          <IptLetter value="Trình độ: " />
                          {item.level}
                        </div>
                        <div>
                          <IptLetter value="Chứng chỉ: " />
                          {item.certificate}
                        </div>
                        <div>
                          <IptLetter value="Điểm số: " />
                          {item.score}
                        </div>
                        <Divider />
                      </div>
              ...
              <Col sm={24} md={24} lg={24} xl={24} xxl={24}>
                <Card title="Học vấn" bordered={false}>
                  {data && data.educations && data.experiences.length > 0 ? (
                    data.experiences.map((item?: IEducation, i?: number) => (
                      <div key={i}>
                        <div>
                          <IptLetter value="Tên trường: " />
                          {item.school}
                        </div>
                        <div>
                          <IptLetter value="Ngành học: " />
                          {item.branchOfLearning}
                        </div>
                        <div>
                          <IptLetter value="Ngày bắt đầu: " />
                          {item.startedDate !== -1 ? (
                            timeConverter(item.startedDate, 1000)
                          ) : (
                            <NotUpdate />
                          )}
                        </div>
                        <div>
                          <IptLetter value="Ngày kết thúc: " />
                          {item.finishedDate !== -1 ? (
                            timeConverter(item.finishedDate, 1000)
                          ) : (
                            <NotUpdate />
                          )}
                        </div>
                        <div>
                          <IptLetter value="Mô tả thêm: " />
                          {item.description}
                        </div>
                        <Divider />
                      </div>
              ...
              <Col sm={24} md={24} lg={24} xl={24} xxl={24}>
                <Card title="Kinh nghiệm" bordered={false}>
                  {data && data.experiences && data.experiences.length > 0 ? (
                    data.experiences.map((item?: IExperience, i?: number) => (
                      <div key={i}>
                        <div>
                          <IptLetter value="Tên công việc: " /> {item.jobName}
                        </div>
                        <div>
                          <IptLetter value="Công ti: " />
                          {item.companyName}
                        </div>
                        <div>
                          <IptLetter value="Ngày bắt đầu: " />
              ...
              <Col sm={24} md={24} lg={24} xl={24} xxl={24}>
                <Card title="Yêu cầu khác" bordered={false}>
                  {data && data.skills && data.skills.length > 0 ? (
                    data.skills.map((item?: ISkill, i?: number) => (
                      <span className="item-more-info" key={i}>
                        {item.name + ", "}
                      </span>
                    ))
                  ) : (
